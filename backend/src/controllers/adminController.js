...
exports.getOrders = async (req, res, next) => {
  try {
    ...
    const [charmDocs, productDocs] = await Promise.all([
      charmIds.size > 0
        ? Charm.find({ _id: { $in: [...charmIds] } }).populate('category', 'name').lean()
        : Promise.resolve([]),
      productIds.size > 0
    ...
    const charmMap = new Map(charmDocs.map((charm) => [String(charm._id), charm]));
    ...
    const enrichedOrders = orders.map((order) => ({
      ...
      items: (order.items || []).map((item) => {
        ...
        const snapshotCharmDetails = Array.isArray(item?.designCharmDetails) ? item.designCharmDetails : [];
        const charmDetails = snapshotCharmDetails.length > 0
          ? snapshotCharmDetails
          : Array.isArray(item?.designCharms)
            ? item.designCharms.map((charmId) => charmMap.get(String(charmId)) || { _id: String(charmId), missing: true }).filter(Boolean)
            : [];

        return {
          ...item,
          productDetail: item?.productType === 'Charm'
    ...
    res.json({ orders: enrichedOrders, total, page: parseInt(page, 10), limit: perPage });
  } catch (err) {
    next(err);
  }
};
